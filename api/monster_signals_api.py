...
"""
Monster Signals API - Simplified and self-contained version
"""
...
# Import our simplified data fetcher
...
def generate_monster_signal(symbol):
    """
    Generate a monster signal with strict filtering criteria
    """
    try:
        logger.info(f"🔍 Analyzing {symbol} with monster filters...")
        ...
        logger.info(f"   RSI: {rsi:.2f}, ATR: {atr:.6f}")
...
def generate_monster_signals():
    """
    Generate monster signals using simplified logic
    """
    try:
        ...
        logger.info(f"Starting monster signal generation for {len(symbols)} symbols")
        ...
        for symbol in symbols:
            try:
                ...
                if signal:
                    ...
                    frontend_signal = {
                        'id': f"monster_{signal['symbol']}_{int(datetime.now().timestamp())}",
...
def health_check():
    """
    Health check endpoint for monster signals API
    """
    try:
        # Test data fetching capability
        test_df = fetch_data('BTCUSDT', '15m', limit=5)
        ...
        return jsonify({
            ...
            'services': {
                'binance_api': 'ok' if not test_df.empty else 'fallback_to_mock',
